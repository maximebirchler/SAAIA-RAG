...
- Treat explicit descriptors in the user request as required evidence. If the hits prove only a head term but do not prove a requested qualifier, say the exact qualified request is not shown in the sources and offer only the partial documented lead. Never copy an unsupported qualifier into a title, component, instruction or conclusion.
