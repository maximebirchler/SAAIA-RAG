...
internal static class ClientUiText
{
    private static readonly Dictionary<string, Dictionary<string, string>> Catalog = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["admin.jobs.card.document_versions"] = Multi("versions (ingestion/indexée) {0}/{1}", "versions (ingestion/indexed) {0}/{1}", "versiones (ingesta/indexada) {0}/{1}", "versões (ingestão/indexada) {0}/{1}", "Versionen (Ingestion/indiziert) {0}/{1}", "versioni (ingestione/indicizzata) {0}/{1}"),
