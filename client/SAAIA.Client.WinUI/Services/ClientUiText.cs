...
internal static class ClientUiText
{
    private static readonly Dictionary<string, Dictionary<string, string>> Catalog = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["admin.jobs.empty.hint_reset"] = Multi("Essaie de réinitialiser les filtres pour revenir à la vue active par défaut.", "Try resetting filters to return to the default active view.", "Intenta restablecer los filtros para volver a la vista activa por defecto.", "Tenta repor os filtros para voltar à vista ativa por defeito.", "Setze die Filter zurück, um zur standardmäßigen aktiven Ansicht zurückzukehren.", "Prova a reimpostare i filtri per tornare alla vista attiva predefinita."),
