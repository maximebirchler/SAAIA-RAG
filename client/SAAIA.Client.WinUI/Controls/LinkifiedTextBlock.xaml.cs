...
public sealed partial class LinkifiedTextBlock : UserControl
{
    ...
    private const double DocLinkNormalAlphaMultiplier = 0.78;
    private const double DocLinkHoverAlphaMultiplier = 1.00;
    private const double DocLinkPressedAlphaMultiplier = 0.70;
    ...
    {
        var normalBrush = MakeAlphaBrush(baseFg, DocLinkNormalAlphaMultiplier);
        var hoverBrush = MakeAlphaBrush(baseFg, DocLinkHoverAlphaMultiplier);
        var pressedBrush = MakeAlphaBrush(baseFg, DocLinkPressedAlphaMultiplier);
        ...
        btn.PointerEntered += (_, _) => tb.Foreground = hoverBrush;
        btn.PointerExited += (_, _) => tb.Foreground = normalBrush;
        btn.PointerPressed += (_, _) => tb.Foreground = pressedBrush;
        btn.PointerReleased += (_, _) => tb.Foreground = hoverBrush;
        btn.PointerCanceled += (_, _) => tb.Foreground = normalBrush;
    ...
    private static void RenderRichTextInto(RichTextBlock rtb, string text, Brush fg, double fs, double linkAlphaMultiplier)
    {
        ...
        var p = new Paragraph();
        var linkFg = MakeAlphaBrush(fg, linkAlphaMultiplier);
