...
WHERE
  ...
  OR (
    CARDINALITY(REGEXP_SPLIT_TO_ARRAY(BTRIM(title), '\s+')) >= 6
    AND LOWER(title) ~ '\b(est|sont|doit|doivent|peut|peuvent|pouvez|permet|permettent|recommande|recommandons|utilisez|utiliser|trouver|trouvez|preparez|préparez|melangez|mélangez|ajoutez|ouvrez|fermez|retirez|servez|is|are|can|must|should|allows?|use|uses|using|prepare|prepared|serves?)\b'
  )
