...
AS $$
  SELECT
    ...
    OR (
      CASE
        WHEN jsonb_typeof(p_metadata #> '{evidence,quantityFacts}') = 'array'
          THEN jsonb_array_length(p_metadata #> '{evidence,quantityFacts}')
        ELSE 0
      END >= 2
      AND CASE
            WHEN COALESCE(p_metadata #>> '{evidence,scaleBasis,count}', '') ~ '^[0-9]+$'
              THEN (p_metadata #>> '{evidence,scaleBasis,count}')::int
            ELSE 0
          END > 0
      AND CASE
