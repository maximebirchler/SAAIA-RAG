...
sealed class IngestionScanner : BackgroundService
{
        ...
        CancellationToken ct)
    {
        ...
        // 0) stale running recovery is centralized in JobRepo so the scanner
        // does not mark a long-running worker job as failed.
        var staleRunningAfter = TimeSpan.FromMinutes(Math.Clamp(opt.StaleRunningMinutes, 5, 24 * 60));
        var requeued = await JobRepo.RequeueStaleRunningAsync(ds, staleRunningAfter, ct);
        if (requeued > 0)
            _log.LogWarning("Scanner requeued {Count} stale running ingestion jobs", requeued);
